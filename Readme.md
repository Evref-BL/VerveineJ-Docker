...
```sh
docker pull badetitou/verveinej:v3.0.4
docker run -v <full/path/toSource>:/src -v [<full/path/toDependency>:/dependency] badetitou/verveinej:v3.0.4 <verveineJOption> .
```
...
```sh
docker run -v D:\Users\benoit.verhaeghe\Documents\git\badetitou\Carrefour\testing\src\:/src badetitou/verveinej:v3.0.4 -format json -o testoutput.json .
```
...
```sh
docker run --rm -v "/mnt/d/Developpement/git/evol-critics:/src" badetitou/verveinej:v3.0.4 -Xmx16g -- -format json -alllocals -anchor assoc -o output.json .
```
